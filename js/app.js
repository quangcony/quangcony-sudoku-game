...
const loadSudoku = () => {
    ...
    game_level.innerHTML = CONSTANT.LEVELS_NAME[game.level];
...
const saveResultHistory = () => {
    let result = {
        level: CONSTANT.LEVELS_NAME[level_index],
        seconds: seconds,
        name: getPlayerName()
    }
    history.push(result);
...
// show result history to ul
const showHistory = () => {

    history = getHistory();
    const htmls = history.sort((a, b) => {return a.seconds - b.seconds}).map((item, index) => {
        return `
        <li>
            <div class="rank">${index + 1}</div>
            <div class="username">${item.name}</div>
            <div class="duration">${showTime(item.seconds)}</div>
            <div class="level">${item.level}</div>
        </li>`;
    })
    history_list.innerHTML = htmls.join('');
...
const startGame = () => {
    ...
    game_level.innerHTML = CONSTANT.LEVELS_NAME[level_index];
...
document.querySelector('#btn-level').addEventListener('click', (e) => {
    ...
    e.target.innerHTML = CONSTANT.LEVELS_NAME[level_index];
})
...
const init = () => {
    ...
    document.querySelector('meta[name="theme-color"]').setAttribute('content', darkmode ? '#1a1s2e' : '#fff');
